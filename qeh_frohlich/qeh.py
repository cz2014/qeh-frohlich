...
solves Poisson equations for induced potentials, and computes screening matrices.
...
- check_building_blocks(): Validates building blocks are on the same grid
...
class Heterostructure:
    ...
    def __init__(self, structure, d, thicknesses=None,
        ...
        for n, name in enumerate(structure):
            if name not in namelist:
                ...
                chi_monopole_el.append(np.array(chim_el[:qindex, :windex]))
                drho_monopole.append(np.array(drhom[:qindex]))
                if include_dipole:
                    chi_dipole.append(np.array(chid[:qindex, :windex]))
                    chi_dipole_el.append(np.array(chid_el[:qindex, :windex]))
        ...
        chi_monopole_el = np.array(chi_monopole_el)[:, self.q1: self.q2]
        chi_dipole = np.array(chi_dipole)[:, self.q1: self.q2]
        chi_dipole_el = np.array(chi_dipole_el)[:, self.q1: self.q2]
        ...
        # Store tri-component chi: full, electronic-only, and lattice
        self.chi_monopole_full = chi_monopole
        self.chi_monopole_el = chi_monopole_el
        self.chi_monopole_lat = chi_monopole - chi_monopole_el
        self.chi_monopole = self.chi_monopole_full  # Backward compatibility
        if include_dipole:
            self.chi_dipole_full = chi_dipole
            self.chi_dipole_el = chi_dipole_el
            self.chi_dipole_lat = chi_dipole - chi_dipole_el
            self.chi_dipole = self.chi_dipole_full  # Backward compatibility
        # Initialize cache for chi_qwij matrices
        self._chi_cache = {}
    ...
    def get_chi_matrix(self, exclude_layers=None):
        """
        Dyson equation: chi_full = chi_intra + chi_intra V_inter chi_full

        Parameters:
        -----------
        exclude_layers : set or list of int, optional
            Layer indices whose lattice polarizability should be excluded from diagonal terms.
            For LO phonon screening in layer i, pass {i} to exclude only that layer's
            lattice self-interaction while keeping lattice from all other layers.
            Default: None (include all lattice polarizabilities)
        """
        # Check cache
        cache_key = tuple(sorted(exclude_layers)) if exclude_layers else None
        if cache_key in self._chi_cache:
            print(f'Using cached chi (exclude_layers={exclude_layers})')
            return self._chi_cache[cache_key]

        print(f'Calculating chi (exclude_layers={exclude_layers})')
        Nls = self.n_layers
        q_abs = self.myq_abs
        # Always start with full chi - will selectively replace per-layer
        chi_m_full_iqw = self.chi_monopole_full
        chi_m_el_iqw = self.chi_monopole_el
        chi_d_iqw = self.chi_dipole  # Dipole unchanged (only monopole affected)
        ...
        for iq in range(nq):
            ...
            for iw in range(0, len(self.frequencies)):
                # Start with full monopole chi for all layers
                chi_intra_i = chi_m_full_iqw[self.layer_indices, iq, iw].copy()

                # Per-layer exclusion: replace with electronic-only for excluded layers
                if exclude_layers:
                    for layer_idx in exclude_layers:
                        if layer_idx < Nls:  # Safety check
                            chi_intra_i[layer_idx] = chi_m_el_iqw[self.layer_indices[layer_idx], iq, iw]

                # Interleave dipole if present (dipole unchanged - only monopole affected)
                if self.chi_dipole is not None:
        ...
        # Cache the result
        self._chi_cache[cache_key] = chi_qwij
        return chi_qwij
    ...
    @timer('Get new matrix (rho to pot)')
    def get_E_matrix(self, step_potential=False, iq_q=None, iw_w=None,
        static=True, exclude_self_lattice=False):
        """
        Get new defined matrix as: E^{-1} = eps^{-1} V

        Parameters:
        -----------
        exclude_self_lattice : bool
            If True, compute invE with per-source lattice exclusion: for charge in
            layer i, exclude layer i's lattice polarizability (to avoid self-interaction)
            while keeping lattice from all other layers. Each column of invE uses a
            different chi matrix. Essential for correct LO phonon screening calculations.
            Default: False (include all lattice polarizabilities everywhere)
        """
        ...
        # Initialize invE matrix
        invE_qwij = np.zeros((nq, nw, self.dim, self.dim), dtype=complex)

        if not exclude_self_lattice:
            # Standard path: compute once with all lattice included
            chi_qwij = self.get_chi_matrix(exclude_layers=None)
            self.chi_qwij = chi_qwij
            ...
            for i, iq in enumerate(iq_q):
                ...
                for j, iw in enumerate(iw_w):
                    ...
                    inveps_ij = np.eye(newkernel_ij.shape[0]) + \
                        np.dot(newkernel_ij, chi_qwij[iq,iw,:,:])
                    invE_qwij[i,j,:,:] = np.dot(inveps_ij, newkernel_ij)
        else:
            # Per-source path: for each source layer, exclude only its lattice
            print("Computing per-source invE with lattice self-exclusion...")
            for source_layer in range(self.n_layers):
                print(f"  Source layer {source_layer}/{self.n_layers-1}")
                # Get chi with ONLY this source layer's lattice excluded
                chi_excl = self.get_chi_matrix(exclude_layers={source_layer})
                ...
                for i, iq in enumerate(iq_q):
                    ...
                    for j, iw in enumerate(iw_w):
                        ...
                        # Compute invE for this source layer's screening
                        inveps_ij = np.eye(newkernel_ij.shape[0]) + \
                            np.dot(newkernel_ij, chi_excl[iq,iw,:,:])
                        E_inv = np.dot(inveps_ij, newkernel_ij)

                        # Determine column index for this source layer
                        # If dipole: monopole at 2*layer, dipole at 2*layer+1
                        # If no dipole: just layer index
                        col_monopole = 2*source_layer if self.chi_dipole is not None else source_layer

                        # Fill ONLY this source's monopole column
                        invE_qwij[i, j, :, col_monopole] = E_inv[:, col_monopole]

                        # If dipole present, also fill dipole column (uses same chi)
                        if self.chi_dipole is not None:
                            col_dipole = 2*source_layer + 1
                            invE_qwij[i, j, :, col_dipole] = E_inv[:, col_dipole]

        # get E^{-1} for full q grid
        invE_fullq = np.zeros((len(self.q_abs), nw, self.dim, self.dim),
            dtype=complex)
        for i in range(self.dim):
            for j in range(self.dim):
                invE_fullq[:,:,i,j] = self.collect_qw(invE_qwij[:,:,i,j])

        if static:
            invE_fullq = invE_fullq[:,0,:,:]

        return self.q_abs/Bohr, self.frequencies*Hartree, invE_fullq
